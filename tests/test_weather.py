...
class TestWeatherSource(unittest.TestCase):
    ...
    @unittest.mock.patch.object(Weather, "query_api", return_value=fake_results)
    ...
    def test_weather_collect_multiple_intervals(self, mock_query):
        ...
        start_date = datetime.date(2016, 1, 1)
        end_date = start_date + datetime.timedelta(days=365*2+365/2) # 2.5 years

        w = Weather(self.config, self.conn)

        expected_intervals = [
            (start_date, start_date+datetime.timedelta(days=365)),
            (start_date+datetime.timedelta(days=365), start_date+datetime.timedelta(days=365*2)),
        ...
        for expected_interval, call_args in itertools.zip_longest(expected_intervals, mock_query.call_args_list):
            args = call_args[0]
